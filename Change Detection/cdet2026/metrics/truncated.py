...
Evaluating Truncated Rankings", SIGIR 2016. This is the tuning instrument (M4).
...
Gain mechanism (TREC Change Detection): document gains are GRADED on the track's scale
0 / 1 / 5 / 10 (not relevant / on-topic / important / vital). The appended terminal
document's gain is the "gain recall" — the fraction of the query's total gain accrued so
far (terminal_gain = sum(retrieved gains) / total gain pool). Pass the assessor's gains
directly; for a binary proxy qrel use gain 1 for every relevant doc.
...
Bounding under graded gains:
  ...
  - RBP'  : the paper's raw rank-biased GAIN RATE. For binary it is in [0,1]; with graded
            gains > 1 it is "expected gain per document" and CAN exceed 1 (by design — it
            is not normalized in the paper). Report it as a rate, or normalize externally.

Inputs to every metric:
    gains      : per-rank gains of the system's ranking, in rank order (0 = non-relevant)
    rel_gains  : the multiset of gains of ALL relevant docs for the query (the gain
                 pool). For binary relevance with R relevant docs this is [1]*R.
                 R = sum(rel_gains); the number of relevant docs is len(rel_gains).

The four metrics reproduce the paper's (binary) Table 1 to three decimals (test_truncated.py);
graded behavior is covered by test_truncated_graded.
"""
...
def rbp_prime(gains: list[float], rel_gains: list[float], p: float = 0.5) -> float:
    ...
    R = sum(rel_gains)
    d = len(gains)
    body = sum((1.0 - p) * g * (p ** (i)) for i, g in enumerate(gains))  # i is 0-based -> p^{i}
    rt = terminal_gain(gains, R)
    return body + rt * (p ** d)


def _ideal_extended(rel_gains: list[float], length: int) -> list[float]:
    """Ideal (d+1)-element gain vector for NDCG'/AP' normalization: the relevant gains
    sorted descending, with an extra terminal gain of 1.0 in the first zero-gain
    position (only if there are fewer than `length` relevant docs)."""
...
def ap_prime(gains: list[float], rel_gains: list[float]) -> float:
    ...
    to [0,1]. The ideal = the relevant gains sorted descending + a terminal gain of 1.0.

    For binary relevance the ideal numerator equals (R+1), so this reproduces the paper's
    Eq. 4 (and Table 1) exactly. For graded gains (0/1/5/10) the (R+1) shortcut is wrong
    (R = sum of gains), which is why raw Eq. 4 can exceed 1; dividing by the ideal fixes it.
    """
    R = sum(rel_gains)
    ext = _extended(gains, R)
    ideal_seq = sorted(rel_gains, reverse=True) + [1.0]  # all relevant gains desc, then terminal 1.0
...
def score_ranking(gains: list[float], rel_gains: list[float], p: float = 0.5) -> dict[str, float]:
    """Convenience: all four truncated metrics for one ranking."""
    return {
